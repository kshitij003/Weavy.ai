...
export function CropNode({ id, data }: NodeProps) {
    ...
    // Local state for manual inputs (fallback if no connection)
    ...
    const handleStart = async () => {
        ...
        try {
            ...
            if (imageEdge) {
                ...
                image = sourceNode?.data?.image as string || sourceNode?.data?.output as string;
            }
            ...
            if (!image) {
                updateNodeData(id, { output: "Error: No image connected." });
                setIsLoading(false);
                return;
            }
        ...
        <div className={cn(
            ...
            <div className="w-full h-full rounded-lg flex flex-col">
                ...
                <div className="p-3 relative">
                    {/* 1. Image Input Handle */}
                    <div className="relative mb-4 group">
                        <div className="flex items-center gap-2 mb-1 justify-between">
                            <span className={cn("text-[10px] font-medium transition-colors", connections.image ? "text-[#E8FF86]" : "text-gray-500")}>
                                Image Input
                            </span>
                    ...
                    <div className="flex flex-col space-y-3 mb-4">
                        <InputField
                            ...
                            handleId="x_percent"
                            value={params.x}
                            onChange={(e: any) => updateParams({ ...params, x: parseFloat(e.target.value) })}
                        ...
                        <InputField
                            ...
                            handleId="y_percent"
                            value={params.y}
                            onChange={(e: any) => updateParams({ ...params, y: parseFloat(e.target.value) })}
                        ...
                        <InputField
                            ...
                            value={params.width}
                            onChange={(e: any) => updateParams({ ...params, width: parseFloat(e.target.value) })}
                        ...
                        <InputField
                            ...
                            value={params.height}
                            onChange={(e: any) => updateParams({ ...params, height: parseFloat(e.target.value) })}
                    ...
                    {data.output && typeof data.output === 'string' && data.output.startsWith("data:image") ? (
                        <div className="mb-3 rounded overflow-hidden border border-[#2A2A2D]">
                            <img src={data.output} alt="Cropped" className="w-full h-auto max-h-32 object-contain bg-black" />
                        </div>
                    ) : (
                        data.output && (
